<template>
  ...
  >
    ...
    <main class="max-w-[1200px] mx-auto px-4 sm:px-6 py-6 relative z-[1]">
      <!-- Page title -->
      <h1 class="text-slate-600 dark:text-white text-[20px] sm:text-[24px] font-bold mb-4">
        {{ t('detail.title') }}
      </h1>
      ...
      <div class="flex items-center gap-2 mb-5 flex-wrap">
        ...
        <div v-if="data" class="flex items-center gap-2 min-w-0">
          <Icon :name="platformIcon" class="w-4 h-4 flex-shrink-0 text-slate-400" />
          <p class="text-slate-700 dark:text-slate-300 text-[13px] font-medium truncate">
            {{ data.serviceName }}
          </p>
          <span
          ...
          <span class="flex items-center gap-1 text-[10px] px-2 py-0.5 rounded-full bg-indigo-500/10 border border-indigo-500/20 text-indigo-400 font-semibold flex-shrink-0">
      ...
      <div v-if="pending" class="flex flex-col items-center justify-center py-24 gap-4">
        ...
        <p class="text-slate-500 text-[13px]">{{ t('detail.loading') }}</p>
      </div>

      <!-- Error -->
      <div v-else-if="error" class="card p-10 text-center">
        <Icon name="heroicons:exclamation-triangle" class="w-10 h-10 text-red-400 opacity-60 mx-auto mb-3" />
        <p class="text-slate-300 font-medium">{{ t('detail.error') }}</p>
        <p class="text-slate-600 text-[12px] mt-1">{{ error.message }}</p>
      </div>

      <!-- No data -->
      <div v-else-if="data && data.total === 0" class="card p-12 text-center">
        <Icon name="heroicons:archive-box" class="w-10 h-10 text-slate-600 mx-auto mb-3" />
        <p class="text-slate-400 font-medium">{{ t('detail.noTransactions') }}</p>
        <p class="text-slate-600 text-[12px] mt-1">{{ t('detail.noTransactionsHint') }}</p>
      </div>
      ...
      <template v-else-if="data">
        ...
        <div class="card p-5 sm:p-6 mb-4">
          <div class="flex flex-col sm:flex-row sm:items-start gap-4">
            <div class="flex-1 min-w-0">
              <p class="text-[11px] font-semibold uppercase tracking-widest text-slate-600 mb-1.5">
                {{ t('detail.serviceStatus') }}
              </p>
              <h1 class="text-slate-900 dark:text-white text-[17px] sm:text-[19px] font-bold leading-snug mb-3">
                {{ data.serviceName }}
              </h1>
              <div class="flex items-center flex-wrap gap-2">
                ...
                <span class="text-[11px] px-2.5 py-1 rounded-full bg-indigo-500/10 border border-indigo-500/20 text-indigo-400 font-medium">
                  {{ data.serviceType || t('detail.smmService') }}
                </span>
                <span class="flex items-center gap-1 text-[11px] text-slate-600">
                  <Icon name="heroicons:rectangle-stack" class="w-3 h-3" />
                  {{ data.total.toLocaleString('id-ID') }} {{ t('detail.transactions') }}
                </span>
              </div>
            </div>
            <!-- Completion rate -->
            <div class="flex-shrink-0 text-right hidden md:block">
              ...
              <p class="text-slate-600 text-[12px] mt-1">
                {{ t('detail.ofTransactions', { n: data.total }) }}
              </p>
            </div>
          </div>
          ...
          <div class="mt-5 flex items-center gap-4">
            <div class="w-full">
              <p class="text-[11px] font-semibold uppercase tracking-widest text-slate-600 mb-2.5">
                {{ t('detail.statusDist') }}
              </p>
              <div class="flex h-2.5 rounded-full overflow-hidden gap-0.5">
                <div v-if="data.completed"   class="bg-emerald-500 transition-all" :style="{ flex: data.completed }"></div>
                <div v-if="data.partial"     class="bg-yellow-500 transition-all"  :style="{ flex: data.partial }"></div>
                <div v-if="data.processing"  class="bg-blue-500 transition-all"    :style="{ flex: data.processing }"></div>
                <div v-if="data.inProgress"  class="bg-violet-500 transition-all"  :style="{ flex: data.inProgress }"></div>
                <div v-if="data.canceled"    class="bg-red-500/70 transition-all"  :style="{ flex: data.canceled }"></div>
                <div v-if="data.total === 0" class="bg-slate-200 dark:bg-white/5 flex-1"></div>
              </div>
              <div class="flex flex-wrap gap-x-4 gap-y-1.5 mt-2.5">
                <span v-if="data.completed"  class="flex items-center gap-1.5 text-[11px] text-slate-500">
                  <span class="w-2 h-2 rounded-full bg-emerald-500 flex-shrink-0"></span>
                  Completed ({{ pct(data.completed) }}%)
                </span>
                <span v-if="data.partial"    class="flex items-center gap-1.5 text-[11px] text-slate-500">
                  <span class="w-2 h-2 rounded-full bg-yellow-500 flex-shrink-0"></span>
                  Partial ({{ pct(data.partial) }}%)
                </span>
                <span v-if="data.processing" class="flex items-center gap-1.5 text-[11px] text-slate-500">
                  <span class="w-2 h-2 rounded-full bg-blue-500 flex-shrink-0"></span>
                  Processing ({{ pct(data.processing) }}%)
                </span>
                <span v-if="data.inProgress" class="flex items-center gap-1.5 text-[11px] text-slate-500">
                  <span class="w-2 h-2 rounded-full bg-violet-500 flex-shrink-0"></span>
                  In Progress ({{ pct(data.inProgress) }}%)
                </span>
                <span v-if="data.canceled"   class="flex items-center gap-1.5 text-[11px] text-slate-500">
                  <span class="w-2 h-2 rounded-full bg-red-500/70 flex-shrink-0"></span>
        ...
        <div class="grid grid-cols-6 lg:grid-cols-5 gap-3 mb-4">
          ...
          >
            ...
            <div class="mt-3 h-1 rounded-full bg-slate-200 dark:bg-white/[0.06] overflow-hidden">
              <div :class="['h-full rounded-full transition-all duration-700', s.barColor]" :style="{ width: pct(s.count) + '%' }"></div>
            </div>
          </div>
        </div>
        ...
        <div class="card p-5 mb-4" v-if="data.orders.length > 0">
          <div class="flex items-center gap-2 mb-4">
            ...
            <span class="text-[14px] font-bold text-slate-900 dark:text-white">{{ t('detail.heatmap') }}</span>
            <span class="text-[11px] text-slate-500 ml-1">{{ t('detail.heatmapSubtitle') }}</span>
          </div>
          <div class="overflow-x-auto">
            <div class="min-w-[560px]">
              ...
              <div class="flex items-center gap-2 mt-3 justify-end">
                <span class="text-[10px] text-slate-600">{{ t('detail.fewOrders') }}</span>
                <div class="flex gap-0.5">
                  <div v-for="l in 5" :key="l" class="w-4 h-4 rounded-sm" :style="{ background: `rgba(99,102,241,${l * 0.18})` }"></div>
                </div>
                <span class="text-[10px] text-slate-600">{{ t('detail.manyOrders') }}</span>
              </div>
            </div>
          </div>
        </div>
        ...
        <div class="card overflow-hidden">
          <div class="px-5 pt-5 pb-3" :style="{ borderBottom: '1px solid var(--border)' }">
            <div class="flex items-center justify-between gap-3 mb-3">
              <div class="flex items-center gap-2">
                <div class="w-1.5 h-5 rounded bg-gradient-to-b from-indigo-500 to-violet-600"></div>
                <span class="text-[14px] font-bold text-slate-900 dark:text-white">{{ t('detail.historyTitle') }}</span>
              </div>
              <span class="text-[11px] text-slate-500">{{ t('detail.historyCount', { n: filteredOrders.length }) }}</span>
            </div>
            <!-- Search -->
            ...
            <table class="w-full border-collapse">
              <thead>
                <tr :style="{ background: 'var(--bg-subtle)' }">
                  <th class="text-left px-5 py-2.5 text-[10px] font-bold text-slate-600 uppercase tracking-widest w-10">#</th>
                  <th class="text-left px-3 py-2.5 text-[10px] font-bold text-slate-600 uppercase tracking-widest">{{ t('detail.colTarget') }}</th>
                  <th class="text-center px-3 py-2.5 text-[10px] font-bold text-slate-600 uppercase tracking-widest w-24">{{ t('detail.colQty') }}</th>
                  <th class="text-center px-3 py-2.5 text-[10px] font-bold text-slate-600 uppercase tracking-widest w-20">{{ t('detail.colRemaining') }}</th>
                  <th class="text-center px-3 py-2.5 text-[10px] font-bold text-slate-600 uppercase tracking-widest w-32">{{ t('detail.colTime') }}</th>
                  <th class="text-center px-3 py-2.5 text-[10px] font-bold text-slate-600 uppercase tracking-widest w-28">Status</th>
                  <th class="text-right px-5 py-2.5 text-[10px] font-bold text-slate-600 uppercase tracking-widest w-32">{{ t('detail.colCompletion') }}</th>
                </tr>
              </thead>
              <tbody class="divide-y divide-slate-100 dark:divide-white/[0.03]">
                ...
                >
                  <td class="px-5 py-3 text-slate-600 text-[12px]">{{ (currentPage - 1) * perPage + i + 1 }}</td>
                  <td class="px-3 py-3">
                    <p v-if="order.link" class="text-slate-700 dark:text-slate-300 text-[12px] truncate max-w-[300px]" title="***">
                      {{ maskLink(order.link) }}
                    </p>
                    <p v-else class="text-slate-600 text-[12px] italic">—</p>
                    <p class="text-slate-600 text-[10px] mt-0.5">ID: {{ maskId(order.id) }}</p>
                  </td>
                  <td class="px-3 py-3 text-center">
                    <span class="text-slate-800 dark:text-slate-200 text-[13px] font-semibold tabular-nums">
                      {{ Number(order.quantity).toLocaleString('id-ID') }}
                    </span>
                  </td>
                  <td class="px-3 py-3 text-center">
                    <span :class="['text-[13px] tabular-nums', (order.remains ?? 0) > 0 ? 'text-yellow-400' : 'text-slate-600']">
                      {{ (order.remains ?? 0).toLocaleString('id-ID') }}
                    </span>
                  </td>
                  <td class="px-3 py-3 text-center">
                    <span class="text-slate-400 text-[12px]">{{ formatDate(order.created_timestamp) }}</span>
                  </td>
                  <td class="px-3 py-3 text-center">
                    <span :class="['text-[11px] px-2 py-1 rounded-full font-semibold border', statusStyle(order.status).cls]">
                      {{ statusStyle(order.status).label }}
                    </span>
                  </td>
                  <td class="px-5 py-3 text-right">
                    <div class="flex items-center justify-end gap-2">
                      <div class="w-16 h-1.5 bg-slate-200 dark:bg-white/[0.05] rounded-full overflow-hidden">
                        <div
                      ...
                      <span :class="['text-[12px] font-semibold tabular-nums w-10 text-right', statusStyle(order.status).textColor]">
                        {{ completionPct(order) }}%
                      </span>
                    </div>
                  </td>
          ...
          <!-- Mobile cards -->
          <div class="md:hidden divide-y divide-slate-100 dark:divide-white/[0.04]">
            <div v-for="(order, i) in paginatedOrders" :key="order.id" class="p-4">
              <div class="flex items-start justify-between gap-3 mb-2">
                <div class="min-w-0 flex-1">
                  <p class="text-slate-600 text-[10px] mb-0.5">
                    ID: {{ maskId(order.id) }} · #{{ (currentPage - 1) * perPage + i + 1 }}
                  </p>
                  <p v-if="order.link" class="text-slate-700 dark:text-slate-300 text-[12px] truncate">
                    {{ maskLink(order.link) }}
                  </p>
                  <p v-else class="text-slate-600 text-[12px] italic">{{ t('detail.noLink') }}</p>
                </div>
                <span :class="['text-[11px] px-2 py-0.5 rounded-full font-semibold border flex-shrink-0', statusStyle(order.status).cls]">
                  {{ statusStyle(order.status).label }}
                </span>
              </div>
              <div class="flex items-center gap-4 text-[12px]">
                <span class="text-slate-500">{{ t('detail.qty') }}
                  <span class="text-slate-700 dark:text-slate-300 font-semibold">{{ Number(order.quantity).toLocaleString('id-ID') }}</span>
                </span>
                <span class="text-slate-500">{{ t('detail.remaining') }}
                  <span :class="(order.remains ?? 0) > 0 ? 'text-yellow-400 font-semibold' : 'text-slate-600'">
                    {{ (order.remains ?? 0).toLocaleString('id-ID') }}
                  </span>
                </span>
                <span class="text-slate-600 ml-auto">{{ formatDate(order.created_timestamp) }}</span>
              </div>
              <div class="flex items-center gap-2 mt-2">
                <div class="flex-1 h-1.5 bg-slate-200 dark:bg-white/[0.05] rounded-full overflow-hidden">
                  <div class="h-full rounded-full" :class="statusStyle(order.status).barColor" :style="{ width: completionPct(order) + '%' }"></div>
                </div>
                <span :class="['text-[11px] font-semibold tabular-nums', statusStyle(order.status).textColor]">
                  {{ completionPct(order) }}%
                </span>
              </div>
            </div>
          ...
          >
            <p class="text-[11px] text-slate-600">
              {{ t('detail.showingOf', {
                ...
                to: Math.min(currentPage * perPage, filteredOrders.length),
          ...
          <div v-if="filteredOrders.length === 0" class="py-12 text-center">
            <Icon name="heroicons:funnel" class="w-8 h-8 text-slate-700 mx-auto mb-2" />
            <p class="text-slate-500 text-[13px]">{{ t('detail.noStatus') }}</p>
          </div>
        </div>
      </template>
    </main>
...
const statusFilters = computed(() => {
  if (!data.value) return []
  return [
    { key: 'all',         label: t('detail.filterAll'),  count: data.value.total },
...
const filteredOrders = computed(() => {
  ...
  if (activeFilter.value !== 'all') {
    list = list.filter(o => {
      ...
      if (activeFilter.value === 'canceled') return s === 'canceled' || s === 'cancelled'
      if (activeFilter.value === 'inprogress') return s === 'inprogress' || s === 'in_progress'
  ...
  if (q) {
    list = list.filter(o =>
      (o.link ?? '').toLowerCase().includes(q) || String(o.id).includes(q)
    )
  }
...
function statusStyle(status: string) {
  ...
  if (s === 'completed')
    return { label: 'Completed',   cls: 'bg-emerald-500/10 border-emerald-500/20 text-emerald-400', barColor: 'bg-emerald-500', textColor: 'text-emerald-400' }
  if (s === 'partial')
    return { label: 'Partial',     cls: 'bg-yellow-500/10  border-yellow-500/20  text-yellow-400',  barColor: 'bg-yellow-500',  textColor: 'text-yellow-400'  }
  if (s === 'processing')
    return { label: 'Processing',  cls: 'bg-blue-500/10    border-blue-500/20    text-blue-400',    barColor: 'bg-blue-500',    textColor: 'text-blue-400'    }
  if (s === 'inprogress' || s === 'in_progress')
    return { label: 'In Progress', cls: 'bg-violet-500/10 border-violet-500/20 text-violet-400',    barColor: 'bg-violet-500',  textColor: 'text-violet-400'  }
  if (s === 'canceled' || s === 'cancelled')
    return { label: 'Canceled',    cls: 'bg-red-500/10 border-red-500/20 text-red-400',             barColor: 'bg-red-500/70',  textColor: 'text-red-400'     }
  return { label: status, cls: 'bg-slate-100 dark:bg-white/5 border-slate-200 dark:border-white/10 text-slate-500 dark:text-slate-400', barColor: 'bg-slate-500', textColor: 'text-slate-400' }
}
...
function formatDate(ts?: number) {
  if (!ts) return '—'
  const locale = lang.value === 'en' ? 'en-US' : 'id-ID'
...
const heatmapDays = computed(() => [0,1,2,3,4,5,6].map(i => t(`day.${i}`)))
